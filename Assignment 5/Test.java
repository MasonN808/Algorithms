...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        for (int[][] i: out.matrices(copy_graphs0)){
            ...
            // sort the source nodes in the matrix
            int[] src_nodes = new int[i.length];
            for (int k = 0; k < src_nodes.length; k++){
                src_nodes[k] = i[k][0];
            }

            // sort the matrix relatively to the source nodes
            RelativeInsertionSort_Decreasing RIS = new RelativeInsertionSort_Decreasing();
            // relatively sort unit_prices and spices
            RIS.relative_insertionSort_increasing(src_nodes, i);
